<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="包间类型" prop="baojianleixing">
          <el-input v-model="ruleForm.baojianleixing" 
              placeholder="包间类型" clearable  :readonly="ro.baojianleixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  computed: {
    zongjiage:{
      get: function () {
        return 1*this.ruleForm.shijiage*this.ruleForm.yuyueshizhang
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `baojianyuding/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.baojianyudingCrossAddOrUpdateFlag = false;
                  this.parent.search();
